  ...
  async execute(): Promise<SaleResponseDto[]> {
    ...
    const salesWithDetails = await Promise.all(
      sales.map(async (sale) => {
        ...
        const sellerStats = await this.transactionRepository.getSellerStats(sale.sellerId);
        ...
        return {
          ...
          sellerSalesCount: sellerStats.salesCount,
          sellerSuccessRate: sellerStats.successRate,
          tcgId: card?.tcgId
        } as SaleResponseDto;
      })
    );
